...
if (isset($_POST['member']) && $_POST['member']=="add") {
  ...
  $mem_address = mysqli_real_escape_string($condb,$_POST["mem_address"]); // Add mem_address
  ...
  if($num > 0) {
      ...
      $sql = "INSERT INTO tbl_member (ref_l_id, mem_name, mem_username, mem_password, mem_address, mem_img)
              VALUES ('$ref_l_id', '$mem_name', '$mem_username', '$mem_password', '$mem_address', '$newname')";
  ...
  $result_del = mysqli_query($condb,$sql_del) or die ("Error Query: $sql_del" . mysqli_error());	
  mysqli_close($condb);
  echo "<script type= 'text/javascript'>"; 
